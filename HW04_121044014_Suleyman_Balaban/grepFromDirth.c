...
void grepFromDirectory(char *directoryName,char *target) {  
    ...
    for (i = 0; i < 1; ++i)
        if((pid=fork())<=0)                 //fork for each directory
            break;
    if(pid==0) {
        for(i = 0; i < numberOfFiles; ++i) {
            if (isdirectory(fileNames[i]))
                grepFromDirectory(fileNames[i],target);
        }
        threadForFiles(directoryName,target,result);

        close(fd[0]);
        write(fd[1],result,256);            //write to pipe
        close(fd[1]);
        exit(1);
    }else {
        while(wait(NULL)!=-1);
        close(fd[1]);
        read(fd[0],string,50);              //read pipe
        close(fd[0]);
        sprintf(fifoResult,"###### dirName :%s #####\n",directoryName);
